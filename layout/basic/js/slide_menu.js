...
$(function(){
    ...
    $('#slideCateList li > a.view').on('click', function(e) {
        var sParam = $(this).attr('cate');
        if (!sParam) return;
        var iCateNo = Number(methods.getParam(sParam, 'cate_no'));
	...
	jQuery('#slide_add_category li').each(function(){
		if( jQuery(this).children('ul').length == 0 ){
            ...
            var catLink = $(this).children('a.view').attr("href");
			jQuery(this).children('a.view').after('<a href="' + catLink  + '" class="cate">상품보기</a>');
		}
    });
